...
static u1 u1g_seesaw_ride_mode = 0;    
static u1 u1g_seesaw_ride_pair_mode = 0;    
...
//                u1g_seesaw_mode = 1;
            ...
            case 4 :  /* 検知後ある一定以上上る */
                ...
                if(s4t_distance < 50){         //ﾍﾞｰｽ100

                    robot.robotSensor.GYRO.gyroOffsetValue = u2_gyro_temp;
                    f4g_forward = 80;
                    chokushin2();
                    
                }else if(s4t_distance < 250){   //べーす３００
...
int
u4g_dispatch_func_seesaw_ride(void)
{
    return u1g_seesaw_ride_mode;
}

int
u4g_dispatch_func_seesaw_ride_pair(void)
{
    return u1g_seesaw_ride_pair_mode;
}
